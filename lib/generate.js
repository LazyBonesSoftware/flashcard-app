...
async function callAnthropic(apiKey, notes, count) {
  // Call our own Next.js API route (server-side) to avoid CORS.
  // The API route forwards the request to Anthropic from the server.
  const resp = await fetch("/api/generate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ apiKey, notes, count }),
  });
